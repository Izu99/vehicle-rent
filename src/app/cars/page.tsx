...
export default function CarsPage() {
    ...
    const loadCars = async () => {
      try {
        ...
        if (filters.transmission && filters.transmission !== '') {
          apiFilters.transmission = filters.transmission === 'Auto' ? 'Automatic' : filters.transmission
        }
  ...
  const transformCarForDisplay = (apiCar: ApiCar) => {
    return {
      ...
      image: apiCar.images.length > 0 ? `http://localhost:5000${apiCar.images[0]}` : null,
      hasImage: apiCar.images.length > 0,
      imageCount: apiCar.images.length,
      price: apiCar.pricePerDay.toString(),
      rating: 4.5,
      reviews: Math.floor(Math.random() * 200) + 50,
      company: apiCar.shopId.shopName,
      features: getCarFeatures(apiCar),
      location: apiCar.shopId.shopAddress.split(',')[0] || 'Unknown',
      ...
      <div className="min-h-screen gradient-background text-gray-900">
        ...
        <div className="flex items-center justify-center min-h-[60vh]">
          <div className="text-center">
            ...
            <Button onClick={() => window.location.reload()} className="gradient-primary gradient-primary-hover text-black">
              Retry
            </Button>
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          ...
          >
            ...
            <p className="text-gray-300 text-lg max-w-2xl mx-auto">
              Choose from {totalCars}+ vehicles from trusted rental partners across Sri Lanka
            </p>
          </motion.div>
        </div>
      </section>
                ...
                <motion.div key={car.id} variants={itemVariants}>
                  <Card className="overflow-hidden shadow-xl hover:shadow-2xl transition-all duration-300 group">
                    <div className="relative overflow-hidden">
                      {car.hasImage ? (
                        ...
                        <div className="w-full h-48 bg-gradient-to-br from-gray-100 to-gray-200 flex items-center justify-center">
                          <div className="text-center text-gray-400">
                            ...
                            <p className="text-sm font-semibold text-gray-600">{car.name}</p>
                      ...
                      <Badge className={`absolute top-4 right-4 ${car.available ? 'bg-green-500' : 'bg-red-500'} text-white font-semibold`}>
